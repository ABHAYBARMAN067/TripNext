...
import { requireAuth, requireHost } from '../../../lib/roles';
...
export async function GET(request) {
    try {
        ...
        if (minRating > 0) {
            ...
            const ratingMap = new Map(avgRatings.map(r => [r._id.toString(), r.avgRating]));
            listings = listings.filter(listing => (ratingMap.get(listing._id.toString()) || 0) >= minRating);
        }
...
export async function POST(request) {
    try {
        ...
        const { title, description, location, images, price, maxGuests, amenities, instantBook } = body;
