...
export async function POST(request) {
    try {
        ...
        if (!listingId || !rating || !comment) {
            return NextResponse.json({ error: 'Listing ID, rating, and comment are required' }, { status: 400 });
        }
        ...
        const existingReview = await Review.findOne({ listing: listingId, author: user.id });
        if (existingReview) {
